...
from Helper.text_helper import drawTextcenter
...
def update_display(
    ...
    drawTextcenter(
        "Time elapsed: " + str(format(time, ".1f")) + "s",
        pygame.font.SysFont("Calibri", 20),
        win,
        100,
        75,
    ...
    drawTextcenter(
        "Number of elements: " + str(number_of_elements),
        pygame.font.SysFont("Calibri", 20),
        win,
        900,
        25,
    ...
    if number_of_elements != -1 and len(height) != 0:
        ...
        for i in range(len(height)):
            ...
            x = Button(
                ...
                HEIGHT,
                -(step),
                -(height[i] / maximum_value) * 3 * HEIGHT / 4,
                BLACK,
                color_height[i],
                str(height[i]),
                int(round(step - 20)),
